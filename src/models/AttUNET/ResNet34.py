...
class ResNet34Encoder(nn.Module):
    def __init__(self, freeze=True):
        ...
        self.conv2 = resnet.layer1  # [B, 64, H/4, W/4]
        self.conv3 = resnet.layer2  # [B, 128, H/8, W/8]
        self.conv4 = resnet.layer3  # [B, 256, H/16, W/16]
        self.conv5 = resnet.layer4  # [B, 512, H/32, W/32]

    def forward(self, x):
        x1 = self.conv1(x)          # [B, 64, H/2, W/2]
        x1m = self.maxpool(x1)      # [B, 64, H/4, W/4]
        x2 = self.conv2(x1m)        # [B, 64, H/4, W/4]
...
class AttUNetResNet34(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, freeze=True):
        super(AttUNetResNet34, self).__init__()
        ...
        self.up1 = Up(512, 256, attn_channels=128)    # 512 -> 256
        self.up2 = Up(256, 128, attn_channels=64)      # 256 -> 128
        self.up3 = Up(128, 64, attn_channels=32)       # 128 -> 64
        self.up4 = Up(64, 64, attn_channels=32)        # 64 -> 64 (special case for ResNet)
    ...
    def forward(self, x):
        # Encoder
        x1, x2, x3, x4, x5 = self.encoder(x)  # x1:64, x2:64, x3:128, x4:256, x5:512
